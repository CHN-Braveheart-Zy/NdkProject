...
#ifndef NDKPROJECT_UTILS_H
#define NDKPROJECT_UTILS_H
...
RTMPPacket *createAudioPacket(int8_t *buf, int len, long tms, int type) {

    RTMPPacket *packet = static_cast<RTMPPacket *>(malloc(sizeof(RTMPPacket)));
    int ret = RTMPPacket_Alloc(packet, len + 2);
...
void prepareVideo(int8_t *buf, int len, long tms, Live *live) {
    for (int i = 0; i < len; ++i) {
        if (i + 4 < len) {
